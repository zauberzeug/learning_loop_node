...
async def test_successful_detecting():
    ...
    assert active_training_module.detections.exists(trainer.training)
...
def test_save_load_detections():
    ...
    trainer = TestingTrainer()
    trainer.training = active_training_module.load()

    active_training_module.detections.save(trainer.training, detections)
    assert active_training_module.detections.exists(trainer.training)

    stored_detections = active_training_module.detections.load(trainer.training)
    assert stored_detections == detections

    active_training_module.detections.delete(trainer.training)
    assert active_training_module.detections.exists(trainer.training) is False
