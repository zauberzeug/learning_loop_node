...
async def test_training_process_is_stopped_when_trainer_reports_error(test_initialized_trainer: TestingTrainer):
...
async def test_log_can_provide_only_data_for_current_run(test_initialized_trainer: TestingTrainer):
    trainer = test_initialized_trainer
    create_active_training_file(trainer, training_state='train_model_downloaded')
    trainer.load_active_training()
    _ = asyncio.get_running_loop().create_task(trainer.train())
